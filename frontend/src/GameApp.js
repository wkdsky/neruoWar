import React, { Suspense, lazy, useState, useEffect, useLayoutEffect, useRef, useCallback } from 'react';
import './App.css';
...
const AlliancePanel = lazy(() => import('./components/game/AlliancePanel'));
...
const App = () => {
    ...
    useEffect(() => {
        ...
        initializeSceneManager().then((cleanup) => {
            if (didCancel) {
                if (cleanup) {
                    cleanup();
                }
            ...
            cleanupResizeListeners = cleanup || null;
        });
