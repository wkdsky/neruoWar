...
const segmentHitAabb = (a, b, minX, maxX, minY, maxY) => {
  const dx = (Number(b?.x) || 0) - (Number(a?.x) || 0);
  const dy = (Number(b?.y) || 0) - (Number(a?.y) || 0);
  let t0 = 0;
  let t1 = 1;
  ...
  if (!clip(-dx, (Number(a?.x) || 0) - minX)) return null;
  if (!clip(dx, maxX - (Number(a?.x) || 0))) return null;
  if (!clip(-dy, (Number(a?.y) || 0) - minY)) return null;
  if (!clip(dy, maxY - (Number(a?.y) || 0))) return null;
...
export const raycastRotatedRect = (start, end, rect, inflate = 0) => {
  ...
  const localStart = rotate2D(sx - cx, sy - cy, -rot);
  const localEnd = rotate2D(ex - cx, ey - cy, -rot);
  const hit = segmentHitAabb(localStart, localEnd, -hw, hw, -hh, hh);
  if (!hit) return null;
  const world = rotate2D(hit.x, hit.y, rot);
  return {
    t: hit.t,
    x: cx + world.x,
    y: cy + world.y,
...
export const raycastObstacles = (start, end, obstacles = [], inflate = 0) => {
  ...
  for (let i = 0; i < obstacles.length; i += 1) {
    ...
    const hit = raycastRotatedRect(start, end, obs, inflate);
    if (!hit) continue;
