...
import { EFFECT_INSTANCE_STRIDE } from '../snapshot/BattleSnapshotSchema';

export { EFFECT_INSTANCE_STRIDE };

const VS = `#version 300 es
layout(location=0) in vec2 aQuadPos;
layout(location=1) in vec2 aUv;
layout(location=2) in vec4 iData0; // x y z radius
layout(location=3) in vec4 iData1; // team type life age
...
void main() {
  ...
  float d = length(p);
  if (d > 1.0) discard;
  float ring = smoothstep(0.95, 0.3, d) - smoothstep(0.52, 0.0, d);
  float alpha = max(0.0, ring) * clamp(vLife, 0.0, 1.0);
  vec3 color = (vTeam < 0.5) ? vec3(0.52, 0.85, 1.0) : vec3(1.0, 0.62, 0.44);
  if (vType > 0.5) {
    color = mix(color, vec3(1.0, 0.86, 0.40), 0.55);
  }
  if (uUseTexArray > 0.5) {
    float layer = floor(mod(vType, 6.0) + 0.5);
    vec4 texel = texture(uTexArray, vec3(vUv, layer));
    color = mix(color, texel.rgb, texel.a * 0.78);
    alpha *= max(0.18, texel.a);
  }
...
export default class EffectRenderer {
  constructor(gl) {
    ...
    this.buffer = createDynamicInstanceBuffer(gl, EFFECT_INSTANCE_STRIDE * 4096);
    this.capacity = 4096;
    this.count = 0;
    this.data = new Float32Array(this.capacity * EFFECT_INSTANCE_STRIDE);
  ...
  bindLayout() {
    ...
    const stride = EFFECT_INSTANCE_STRIDE * 4;
  ...
  ensureCapacity(count) {
    ...
    this.buffer = createDynamicInstanceBuffer(this.gl, this.capacity * EFFECT_INSTANCE_STRIDE);
    this.bindLayout();
  }

  updateFromSnapshot(effects) {
    const count = Math.max(0, Number(effects?.count) || 0);
    this.ensureCapacity(count);
    if (count <= 0 || !(effects?.data instanceof Float32Array)) {
      this.count = 0;
      return;
    }
    const len = count * EFFECT_INSTANCE_STRIDE;
    this.data.set(effects.data.subarray(0, len), 0);
    this.count = count;
