...
const EMPTY_CAMERA_ANCHOR = Object.freeze({ x: 0, y: 0, vx: 0, vy: 0, squadId: '', team: '' });
...
const buildDefaultTrainingSkillSlots = (units = {}, unitTypeMap = new Map()) => {
  const categories = getGroupSkillTreeCategories(units, unitTypeMap);
  if (categories.length <= 0) return normalizeSkillSlots([]);
  const selectedCategories = categories.length >= 3
    ? categories.slice(0, 3)
    : categories.length === 2
      ? [categories[0], categories[0], categories[1]]
      : [categories[0], categories[0], categories[0]];

  const slots = selectedCategories.map((treeCategory, slotIndex) => ({
    slotIndex,
    treeCategory,
    skillId: getSkillTreeFirstActiveSkill(treeCategory)?.id || '',
    cooldownRemain: 0
  }));
  return normalizeSkillSlots(slots);
};
...
const buildSmoothStaticColliderParts = (obstacle = {}, collider = null) => {
  if (!collider || typeof collider !== 'object') return [];
  const rotation = Number(obstacle?.rotation) || 0;
  const height = Math.max(1, Number(collider?.h) || Number(obstacle?.height) || 1);
  if (collider.kind === 'circle') {
    const offset = rotate2D(Number(collider?.cx) || 0, Number(collider?.cy) || 0, rotation);
    const radius = Math.max(0.5, Number(collider?.r) || 0.5);
    return [{
      cx: (Number(obstacle?.x) || 0) + offset.x,
      cy: (Number(obstacle?.y) || 0) + offset.y,
      cz: height * 0.5,
      w: radius * 2,
      d: radius * 2,
      h: height,
  ...
  if (collider.kind !== 'compositeCapsule') return [];
  return (Array.isArray(collider?.parts) ? collider.parts : []).map((part) => {
    const startOffset = rotate2D(Number(part?.ax) || 0, Number(part?.ay) || 0, rotation);
    const endOffset = rotate2D(Number(part?.bx) || 0, Number(part?.by) || 0, rotation);
    const start = {
      x: (Number(obstacle?.x) || 0) + startOffset.x,
      y: (Number(obstacle?.y) || 0) + startOffset.y
    };
    const end = {
      x: (Number(obstacle?.x) || 0) + endOffset.x,
      y: (Number(obstacle?.y) || 0) + endOffset.y
    };
    const dx = end.x - start.x;
    const dy = end.y - start.y;
    const radius = Math.max(0.5, Number(part?.r) || 0.5);
    const partHeight = Math.max(1, Number(part?.h) || height);
    return {
      cx: (start.x + end.x) * 0.5,
      cy: (start.y + end.y) * 0.5,
      cz: partHeight * 0.5,
      w: Math.hypot(dx, dy) + (radius * 2),
      d: radius * 2,
      h: partHeight,
...
const refreshObstacleGeometry = (obstacle = {}, itemDef = {}) => {
  ...
  const staticColliderKind = obstacle?.mapStatic === true
    ? String(normalizedItem?.collider?.kind || '')
    : '';
  const preservesSmoothStaticCollider = staticColliderKind === 'circle'
    || staticColliderKind === 'compositeCapsule';
  const runtimeCollider = preservesSmoothStaticCollider ? normalizedItem.collider : geometry.collider;
  ...
  obstacle.collider = runtimeCollider;
  ...
  obstacle.colliderParts = preservesSmoothStaticCollider
    ? buildSmoothStaticColliderParts(obstacle, runtimeCollider)
    : buildWorldColliderParts(obstacle, normalizedItem, {
...
const buildDeployGroupInfo = (group = null, unitTypeMap = new Map(), repConfig = {}) => {
  ...
  return {
    ...
    name: limitNameByDisplayWidth(String(group?.name || '未命名部队')) || '未命名部队',
    totalCount: Math.max(0, Math.round(totalCount)),
    composition,
...
const buildCardComposition = (remainUnits = {}, startUnits = {}, unitTypeMap = new Map()) => {
  ...
  const unitTypeIds = Array.from(new Set([...Object.keys(startMap), ...Object.keys(remainMap)]));
  return unitTypeIds
    .map((unitTypeId) => {
      const unitType = unitTypeMap.get(unitTypeId) || {};
      const startCount = Math.max(0, Math.round(Number(startMap[unitTypeId]) || 0));
      const count = Math.max(0, Math.round(Number(remainMap[unitTypeId]) || 0));
      return {
        unitTypeId,
        unitName: unitType?.name || unitTypeId,
        classTag: inferClassFromUnitType(unitType),
        count,
        startCount,
        percent: startCount > 0 ? (count / startCount) * 100 : 0
      };
    })
    .filter((row) => row.startCount > 0 || row.count > 0)
    .sort((left, right) => right.count - left.count || left.unitName.localeCompare(right.unitName, 'zh-Hans-CN'));
};

const buildCardDetails = (source = {}, unitTypeMap = new Map(), isBattle = false) => {
  const startUnits = normalizeUnitsMap(source?.units || {});
  const remainUnits = isBattle
    ? normalizeUnitsMap(source?.remainUnits || startUnits)
    : startUnits;
  const remainingMetrics = isBattle ? buildCompositionMetrics(remainUnits, unitTypeMap) : null;
  const metrics = isBattle
    ? {
      totalHp: Math.max(0, Math.round(Number(source?.health) || remainingMetrics?.totalHp || 0)),
  ...
  return {
    ...
    formationRect: source?.formationRect && typeof source.formationRect === 'object'
      ? {
        width: Math.max(0, Number(source.formationRect.width) || 0),
        depth: Math.max(0, Number(source.formationRect.depth) || 0),
        spacing: Math.max(0, Number(source.formationRect.spacing) || 0)
      }
      : null,
    templateName: limitNameByDisplayWidth(String(source?.templateName || '').trim())
  };
};
...
const buildSavedDeployGroups = (deployUnits, team, field, unitTypeMap, { defaultSkillSlots = false } = {}) => {
  ...
  source.forEach(({ rawGroup }, index) => {
    ...
    groups.push({
      ...
      name: limitNameByDisplayWidth(String(rawGroup?.name || rawGroup?.templateName || `${safeTeam === TEAM_DEFENDER ? '敌方' : '我方'}部队${index + 1}`).trim()),
      units,
...
const buildDefenderDeployGroups = (
  defenderUnits,
  defenderDeployments,
  field,
  unitTypeMap,
  { defaultSkillSlots = false } = {}
) => {
  ...
  sorted.forEach((deploy, index) => {
    ...
    groups.push({
      ...
      team: TEAM_DEFENDER,
      name: limitNameByDisplayWidth(
        (typeof deploy?.name === 'string' && deploy.name.trim()) ? deploy.name.trim() : `守军${index + 1}`
      ),
      units: assigned,
      ...
      skillSlots: Array.isArray(deploy?.skillSlots)
        ? deploy.skillSlots.slice(0, 3).map((slot) => ({ ...slot }))
        : (defaultSkillSlots ? buildDefaultTrainingSkillSlots(assigned, unitTypeMap) : []),
  ...
  if (remain.length > 0) {
    ...
    remain.forEach((entry, index) => {
      ...
      groups.push({
        ...
        skillSlots: defaultSkillSlots
          ? buildDefaultTrainingSkillSlots({ [entry.unitTypeId]: entry.count }, unitTypeMap)
          : [],
        x,
        y,
        placed: true
...
const createSquad = ({
  ...
  return {
    ...
    minionPathCorridorWidth: Math.max(0, Number(group?.minionPathCorridorWidth) || 0),
...
export default class BattleRuntime {
  ...
  setHoveredDeployGroup(groupId = '') {
    ...
    const group = requestedId ? this.getDeployGroupById(requestedId, TEAM_ANY) : null;
    const nextId = group && group.placed !== false ? group.id : '';
    if (this.hoveredDeploySquadId === nextId) return false;
    this.hoveredDeploySquadId = nextId;
    return true;
  }
  ...
  createDeployGroup(team = TEAM_ATTACKER, {
    ...
    const groupName = limitNameByDisplayWidth(
      (typeof name === 'string' && name.trim())
        ? name.trim()
        : `${candidateName}${targetGroups.length + 1}`
    ) || `部队${targetGroups.length + 1}`;
  ...
  setTrainingBattleSquadControlMode(squadId = '', controlMode = CONTROL_MODE_USER) {
    ...
    if (!squad || squad.remain <= 0) return { ok: false, reason: '未找到可切换的部队' };
    if (squad.isMinionWaveUnit === true || squad.team === TEAM_NEUTRAL) {
      return { ok: false, reason: '该部队由训练场 AI 固定控制' };
    }
  ...
  pickDeployGroup(worldPoint, team = TEAM_ANY, radius = 26) {
    if (worldPoint?.valid === false || !Number.isFinite(Number(worldPoint?.x)) || !Number.isFinite(Number(worldPoint?.y))) {
      return null;
    }
    const safeTeam = typeof team === 'string' ? team.trim() : '';
    ...
    targetGroups.forEach((group) => {
      ...
      const hitPadding = Math.max(8, Number(radius) || 0);
      const width = Math.max(0, Number(rect.width) || 0);
      const depth = Math.max(0, Number(rect.depth) || 0);
      const facing = Number.isFinite(Number(rect.facingRad)) ? Number(rect.facingRad) : 0;
      const frontX = Math.cos(facing);
      const frontY = Math.sin(facing);
      const sideX = -frontY;
      const sideY = frontX;
      const localSide = (dx * sideX) + (dy * sideY);
      const localFront = (dx * frontX) + (dy * frontY);
      const insideFormation = width > 0 && depth > 0
        && Math.abs(localSide) <= (width * 0.5) + hitPadding
        && Math.abs(localFront) <= (depth * 0.5) + hitPadding;
      const footprintRadius = Math.hypot(width, depth) * 0.5;
      const pickRadius = Math.max(hitPadding, footprintRadius + (hitPadding * 0.35));
      if ((insideFormation || dist <= pickRadius) && dist < bestDist) {
        best = group;
        bestDist = dist;
      }
    });
  ...
  buildTrainingMinionPath(team = TEAM_ATTACKER, laneId = 'top', barracksLane = '', requestedExitId = '') {
    ...
    const terrainRegions = Array.isArray(this.trainingMap?.terrainRegions)
      ...
      : [];
    const roadTerrain = terrainRegions.find((region) => (
      String(region?.type || '') === 'road'
      && String(region?.laneId || region?.sourceRouteId || '') === String(lane.id || laneId)
      && (
        String(region?.roadRole || '') === 'main'
        || String(region?.id || '') === `terrain-road-${String(lane.id || laneId)}`
      )
    )) || null;
    const corridorWidth = Math.max(
      24,
      Number(roadTerrain?.height)
        || Number(lane?.visualWidth)
        || Number(lane?.width)
        || 96
    );
    ...
    const resolveSideRamp = (routeTeam, highlandLane, routeExitId) => {
      ...
      for (let index = 0; index < rampPoints.length; index += 1) {
        ...
        if (!inwardEdge || distance < inwardEdge.distance) inwardEdge = { midpoint, distance };
        if (!outwardEdge || distance > outwardEdge.distance) outwardEdge = { start, end, distance };
      }
      if (!inwardEdge || !outwardEdge) return [];
      const outwardPoint = {
        x: (outwardEdge.start.x + outwardEdge.end.x) * 0.5,
        y: (outwardEdge.start.y + outwardEdge.end.y) * 0.5
      };
      const centerPoint = rampPoints.reduce((center, point) => ({
        x: center.x + (point.x / rampPoints.length),
        y: center.y + (point.y / rampPoints.length)
      }), { x: 0, y: 0 });
      return [inwardEdge.midpoint, centerPoint, outwardPoint].filter((point, index, points) => (
        index === 0
        || Math.hypot(
    ...
    const sourceRampPassage = resolveSideRamp(
      team,
      safeBarracksLane,
      exitId
    );
    const targetRampPassage = resolveSideRamp(
      enemyTeam,
      safeBarracksLane,
      exitId
    );
    const sourceRampRoadPoint = sourceRampPassage[sourceRampPassage.length - 1] || null;
    const targetRampRoadPoint = targetRampPassage[targetRampPassage.length - 1] || null;
    const resolveConnector = (sourcePoints, highlandLane, rampEntryPoint) => {
      ...
      if (safeLane === 'mid') {
        ...
        const firstRoadDistance = Math.abs(
          (Number(sourcePoints[0]?.y) || 0) - (Number(lane?.centerY) || 0)
        );
        const lastRoadDistance = Math.abs(
          (Number(sourcePoints[sourcePoints.length - 1]?.y) || 0) - (Number(lane?.centerY) || 0)
        );
        const fromRoad = firstRoadDistance <= lastRoadDistance
          ? sourcePoints
          : sourcePoints.slice().reverse();
        return {
          toRoad: fromRoad.slice().reverse(),
          fromRoad
        };
      }
      return {
    ...
    const sourceConnector = resolveConnector(sourceConnectorPoints, safeBarracksLane, sourceRampRoadPoint);
    const targetConnector = resolveConnector(targetConnectorPoints, safeBarracksLane, targetRampRoadPoint);
    const sourceBarracksPoint = { x: Number(barracks.x) || 0, y: Number(barracks.y) || 0 };
    const spawnPoint = (() => {
      const sourceRampInsidePoint = sourceRampPassage[0] || null;
      if (!sourceRampInsidePoint) {
      ...
      const entryVector = {
        x: sourceRampInsidePoint.x - sourceBarracksPoint.x,
        y: sourceRampInsidePoint.y - sourceBarracksPoint.y
      };
    ...
    const connectorThreshold = Math.max(24, (Number(lane?.width) || 24) * 0.35);
    const sourceNeedsConnector = sourceRampPassage.length > 0
      || Math.abs(sourceBarracksPoint.y - (Number(lane?.centerY) || 0)) > connectorThreshold;
    sourceRampPassage.forEach(appendPoint);
    if (sourceNeedsConnector) sourceConnector.toRoad.forEach(appendPoint);
    const roadLine = team === TEAM_DEFENDER ? [...visualCenterline].reverse() : visualCenterline;
    const forwardSign = team === TEAM_DEFENDER ? -1 : 1;
    roadLine.forEach((point) => {
      if (
        !sourceNeedsConnector
        && (((Number(point?.x) || 0) - spawnPoint.x) * forwardSign) < -1
      ) return;
      appendPoint(point);
    });
    const targetBarracks = this.resolveTrainingMinionBarracks(enemyTeam, safeLane, safeBarracksLane);
    const targetNeedsConnector = targetRampPassage.length > 0
      || Math.abs((Number(targetBarracks?.y) || 0) - (Number(lane?.centerY) || 0)) > connectorThreshold;
    if (targetNeedsConnector) targetConnector.fromRoad.forEach(appendPoint);
    targetRampPassage.slice().reverse().forEach(appendPoint);
    ...
    const spawnTeamWave = (team) => {
      spawnSpecs.forEach((spec) => {
        ...
        const group = {
          ...
          minionPathCorridorWidth: path.corridorWidth,
  ...
  startBattle() {
    ...
    const selectedTrainingSquad = this.isTrainingMode && selectedDeployGroupId
      ? this.sim.squads.find((row) => (
        row?.sourceDeployGroupId === selectedDeployGroupId && (Number(row?.remain) || 0) > 0
      )) || null
      : null;
  ...
  getSquadCameraAnchor(squadId = '') {
    ...
    const renderedAnchor = this.getRenderedBattleSquadAnchor(squadId);
    return {
      x: Number.isFinite(Number(renderedAnchor?.centerX))
        ? Number(renderedAnchor.centerX)
        : (Number.isFinite(Number(squad.centerX)) ? Number(squad.centerX) : (Number(squad.x) || 0)),
      y: Number.isFinite(Number(renderedAnchor?.centerY))
        ? Number(renderedAnchor.centerY)
        : (Number.isFinite(Number(squad.centerY)) ? Number(squad.centerY) : (Number(squad.y) || 0)),
  ...
  pickSquadAtPoint(worldX, worldY, options = {}) {
    ...
    for (let i = 0; i < squads.length; i += 1) {
      ...
      const renderedAnchor = this.getRenderedBattleSquadAnchor(row.id);
      const rowX = Number.isFinite(Number(renderedAnchor?.centerX))
        ? Number(renderedAnchor.centerX)
        : (Number.isFinite(Number(row.centerX)) ? Number(row.centerX) : (Number(row.x) || 0));
      const rowY = Number.isFinite(Number(renderedAnchor?.centerY))
        ? Number(renderedAnchor.centerY)
        : (Number.isFinite(Number(row.centerY)) ? Number(row.centerY) : (Number(row.y) || 0));
      const dx = rowX - safeX;
      const dy = rowY - safeY;
      const pickRadius = Math.max(
        12,
        Number(renderedAnchor?.radius) || Number(row.displayRadius) || Number(row.radius) || 12
      );
  ...
  applyOrderToSquad(squad, orderType, safePoint, targetSpec = {}) {
    ...
    const stopAfterTarget = targetSpec?.stopAfterTarget === true;
  ...
  resolveRawFocusAnchor() {
    if (!this.sim || !this.crowd) return EMPTY_CAMERA_ANCHOR;
    ...
    if (!preferred || (Number(preferred.remain) || 0) <= 0) return EMPTY_CAMERA_ANCHOR;
    return {
      x: Number.isFinite(Number(preferred.centerX)) ? Number(preferred.centerX) : (Number(preferred.x) || 0),
      y: Number.isFinite(Number(preferred.centerY)) ? Number(preferred.centerY) : (Number(preferred.y) || 0),
      vx: Number(preferred.vx) || 0,
      vy: Number(preferred.vy) || 0,
      squadId: preferred.id,
      team: preferred.team
    };
  }
  ...
  commandMove(squadId, worldPoint, options = {}) {
    ...
    this.applyOrderToSquad(squad, orderType, routeTarget, {
      ...
      stopAfterTarget: options?.stopAfterTarget
    });
  ...
  getCardRows({ includeMinionWaves = false } = {}) {
    ...
    const squads = this.phase === 'battle' || this.phase === 'ended'
      ...
      : [
        ...this.attackerDeployGroups.map((group, index) => ({
          ...
          ...buildCardDetails(group, this.unitTypeMap, false)
        })),
    ...
    return squads.map((squad) => {
      ...
      radius: Math.max(0, Number(squad.displayRadius) || Number(squad.radius) || 0),
      contactRadius: Math.max(0, Number(squad.contactRadius) || Number(squad.displayRadius) || Number(squad.radius) || 0),
      ...
      ...(this.phase === 'battle'
        ? buildCardDetails(squad, this.unitTypeMap, true)
        : {
          unitComposition: Array.isArray(squad?.unitComposition) ? squad.unitComposition : [],
          formationName: squad?.formationName || '默认阵型',
          formationId: String(squad?.formationId || '').trim(),
  ...
  getFocusAnchor() {
    ...
    if (!this.sim || !this.crowd) return null;
    const focusedSquadId = String(this.focusSquadId || this.selectedBattleSquadId || '');
    const focusedSquad = focusedSquadId ? this.getSquadById(focusedSquadId) : null;
    if (!focusedSquad || (Number(focusedSquad.remain) || 0) <= 0) return null;
    if (String(this.cameraAnchor?.squadId || '') !== focusedSquadId) return null;
    return {
