...
const step = (sim, crowd, dt) => {
  ...
  const revealRadiusBySquad = new Map();
      ...
      interactions.forEach((interaction) => {
        ...
        if (kind === 'concealment') {
          if (isInside) {
            ...
            revealRadiusBySquad.set(squad.id, Math.max(revealRadiusBySquad.get(squad.id) || 0, revealRadius));
          }
        ...
        if (kind === 'trapStagger') {
          ...
          const cooldownSec = Math.max(0.1, Number(interaction?.params?.cooldownSec) || 4);
        ...
        if (kind === 'spotterAura') {
          if (isInside) {
            const bonus = Math.max(0, Number(interaction?.params?.revealBonusRadius) || 0);
            if (bonus > 0) {
              revealRadiusBySquad.set(squad.id, Math.max(revealRadiusBySquad.get(squad.id) || 0, 2.2 + bonus));
            }
          }
          prev.inside = isInside;
          contacts.set(key, prev);
  ...
  squads.forEach((squad) => {
    ...
    const revealRadius = revealRadiusBySquad.get(squad.id) || 2.2;
    if (squad.team === 'defender') {
      squad.hiddenFromAttacker = !shouldRevealHiddenSquad(squad, attackers, revealRadius);
      return;
    }
    if (squad.team === 'attacker') {
      squad.hiddenFromDefender = !shouldRevealHiddenSquad(squad, defenders, revealRadius);
    }
  });
