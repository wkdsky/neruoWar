/**
 * Lane-minion squad and agent AI implementation.
...
import { isTrainingMinionSquad } from './TrainingSquadKind';
...
const resolveAgentEdgeDistance = (agent = {}, target = {}) => Math.max(
  0,
  distanceBetween(agent, target)
    - Math.max(0, finiteNumber(agent?.radius, 2.25))
    - Math.max(0, finiteNumber(target?.radius, 2.25))
);

const resolveBuildingEdgeDistance = (agent = {}, building = {}) => Math.max(
  0,
  distanceBetween(agent, building)
    - resolveBuildingRadius(building)
    - Math.max(0, finiteNumber(agent?.radius, 2.25))
);

/**
 * Per-minion tactical decision.  The wave controller owns target assignment;
 * this function only turns that assignment into the individual agent directive
 * consumed by the shared crowd movement/combat executor.
 */
export const resolveMinionSquadAgentCombatDirective = ({
  agent = {},
  squad = {},
  sim = {},
  crowd = {},
  ...
  if (agent.unitCategory === 'support') {
    ...
    if (!targetAgent || targetAgent.dead || !targetSquad || finiteNumber(targetSquad?.remain) <= 0) {
      agent._combatDirective = null;
      return null;
    }
    const hostile = String(agent?.unitSubtype || '') === 'intervention';
    const castRange = clamp(Math.max(36, finiteNumber(attackRange?.max)), 36, 88);
    if (typeof castSupportEffect === 'function') {
      castSupportEffect({
        agent,
        squad,
        targetSquad,
  ...
  if (
    ...
    && !targetAgent.dead
    && targetSquad
    && finiteNumber(targetSquad?.remain) > 0
    && String(targetSquad.id || '') === String(minionAi?.targetKind === TARGET_KIND_SQUAD ? minionAi.targetId : '')
  ) {
    const directive = {
      kind: 'enemy-agent',
      target: targetAgent,
      distance: distanceBetween(agent, targetAgent),
      edgeDistance: resolveAgentEdgeDistance(agent, targetAgent),
      attackRange,
      minionAssigned: true
    };
    agent._combatDirective = directive;
  ...
  const building = targetBuildingId
    ...
    : null;
  if (building && minionAi?.targetKind === TARGET_KIND_BUILDING && String(minionAi.targetId || '') === targetBuildingId) {
    const edgeDistance = resolveBuildingEdgeDistance(agent, building);
    const directive = {
      kind: 'enemy-building',
      target: building,
      distance: edgeDistance,
      edgeDistance,
      targetRadius: resolveBuildingRadius(building),
      attackRange,
      minionAssigned: true
    };
    agent._combatDirective = directive;
...
export const restoreMinionSquadHoldAnchor = (squad = {}) => {
  if (
    !isTrainingMinionSquad(squad)
    || squad?._minionAi?.state !== MINION_WAVE_AI_STATE.ATTACK_HOLD
    || !squad?._minionAi?.holdAnchor
  ) return;
  squad.x = finiteNumber(squad._minionAi.holdAnchor.x);
  squad.y = finiteNumber(squad._minionAi.holdAnchor.y);
  squad.vx = 0;
  squad.vy = 0;
  squad.speed = 0;
};
...
export const clearMinionWaveAiState = (squad = null, agents = []) => {
  if (!squad || typeof squad !== 'object') return;
  delete squad._minionAi;
  delete squad.minionAiState;
  delete squad._minionNavigationRoute;
  (Array.isArray(agents) ? agents : []).forEach((agent) => {
    if (!agent) return;
    delete agent._minionAi;
    delete agent._minionForwardFloor;
  });
};
...
export const updateMinionWaveAiFrame = ({
  ...
  const minionSquads = (Array.isArray(sim?.squads) ? sim.squads : []).filter((squad) => (
    isTrainingMinionSquad(squad) && finiteNumber(squad?.remain) > 0
  ));
