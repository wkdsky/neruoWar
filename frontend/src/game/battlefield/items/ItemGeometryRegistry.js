  ...
  (Array.isArray(itemType?.sockets) ? itemType.sockets : [])
    .map((socket, index) => {
      const socketId = typeof socket?.socketId === 'string' && socket.socketId.trim()
        ...
        : `socket_${index + 1}`;
      const localPose = socket?.localPose && typeof socket.localPose === 'object'
        ? socket.localPose
        : (socket?.local && typeof socket.local === 'object' ? socket.local : {});
      return {
        socketId,
...
export const getItemGeometry = (itemType = {}) => {
  ...
  return {
    ...
    previewBuilder: (_scene, paletteOverride = null, hintsOverride = null) => createPreviewMesh(itemType, {
      paletteOverride,
      hintsOverride
    }),
    sockets,
